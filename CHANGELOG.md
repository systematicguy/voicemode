...
- **Native Windows support — VoiceMode runs without WSL (#232)** — Windows users no longer need WSL2 (whose audio path has known reliability problems — #341, #342, #98): the MCP server, the `converse` tool, and the CLI now run natively on Windows. Four Unix-only mechanisms were made portable: (1) the conch's `fcntl.flock` file locking moved behind a new `voice_mode/file_lock.py` (unchanged `flock` on Unix; `msvcrt.locking` on a high-offset byte on Windows, since Windows region locks are mandatory and would otherwise block payload reads — the SQLite trick); (2) five `os.kill(pid, 0)` liveness probes became `psutil.pid_exists()` — on Windows `os.kill` is `TerminateProcess` for any signal, so "checking" a lock holder's PID could kill an unrelated live process; (3) the silence-detection recording loop gained a wall-clock cap so a stream that stops delivering audio can no longer listen past `listen_duration_max` forever; (4) `VOICEMODE_DEBUG=true` no longer crashes converse on Windows (`resource.getrusage` → psutil). No behavior change on Linux/macOS — the conch suite passes 98/98 on both Windows 11 and Linux. Works with any OpenAI-compatible local STT/TTS endpoints (e.g. speaches + kokoro-fastapi in Docker).

- **Multi-voice `converse` — speak an ordered sequence gap-free in one call (VM-1772)** — `converse` accepts an optional ordered **`turns`** list so a single call speaks a multi-voice script with **no synthesis dead-air between utterances**. A producer/consumer pipeline **synthesizes turn N+1 while turn N is still playing**, so after the first turn playback is continuous — the only inter-utterance gap is the configured pause, not the per-line TTS round-trip you paid serially before. Each turn is a flat object `{"say": str, "voice"?, "pause_after_ms"?, "tts_instructions"?, "speed"?}`; a turn's `voice` overrides the call-level voice, so `milkshake.sh`'s four separate `voicemode converse` processes (four cold starts, four round-trips, dead air between each) collapse to **one call** that speaks all four lines, each in its own voice. Pacing is a per-turn / call-level **`pause_after_ms`** (default **150 ms**, `0` = gap-free). The scalar `message`/`voice` path is **unchanged** and back-compatible (`message` is now optional); when both `message` and `turns` are given, **`turns` wins**. The conch is held once for the whole sequence (one agent turn — no per-line ordering race). A per-turn TTS failure is logged and skipped rather than aborting the run. Two CLI surfaces lower to the same list: repeatable **`--say 'VOICE:TEXT'`** (order preserved; `VOICE:` optional) for quick lines, and **`--script FILE|-`** (a JSON array of turns, `-` = stdin, heredoc-friendly) for richer/reusable scripts or text containing `:`. **Speak-only** in this release — reply collection (`ask`) and audio-file (`play`) turns are reserved for later phases. See `voicemode converse --help` for examples.
- **Control channel — `skip-forward`: cut the assistant off and take your turn, with a key (VM-1739)** — A new transport command on the control channel: while VoiceMode is speaking, `voicemode control skip-forward` (or the `{"command": "skip_forward"}` wire line) **ends the current utterance instantly and advances straight to your record/listen turn** — the same outcome as talking over the assistant, but triggered deterministically by a button/key/keyword instead of voice-activity detection. Unlike `stop` (which ends the turn and returns a `[control: stop]` marker), `skip_forward` hands the turn to you: `converse` plays the listening chime, records, and returns your response as an ordinary `Voice response: ...` — to the agent it's indistinguishable from letting the utterance finish and then answering. In speak-only mode it simply ends the utterance. It pairs with skip-back as the `|<< >>|` transport, carries no agent-facing text, and rides the same off-by-default, local-only, peer-credential-gated channel as the rest of the control commands. The `>>|` button lands within ~200 ms (same instant-cut path as `stop`) on both Kokoro and mlx-audio. See [docs/reference/control-channel.md](docs/reference/control-channel.md#skip-forward-behaviour).
- **Control channel — `skip-forward` now also ends your *record* turn: one universal advance button (VM-1754)** — `skip_forward` was a playback-phase control (VM-1739): pressing `>>|` while the assistant spoke barged out of the utterance and handed you the mic. It did nothing while VoiceMode was *listening*. Now it works in **both** phases — the same `>>|` button (Stream Deck key or rebound media-key, VM-1742) always means "move on". Pressed **while recording your reply**, it **ends the recording immediately**, transcribes whatever was captured, and returns it as an ordinary `Voice response: ...` — a manual **"I'm done, go now" end-of-turn**, and a reliable **VAD fallback** for when silence detection isn't cutting off cleanly when you finish (especially valuable hands-busy, e.g. while driving). It stays distinct from its siblings: **not** a `[control: stop]` marker (that's `stop`, which ends the turn with no response) and **not** a replay (that's `skip_back`). **Edge case:** press it before you've said anything and the turn advances gracefully with *no speech detected* (success — no error, no hang), rather than replaying or erroring. No new command, CLI, or wire surface — it consumes the existing `skip_forward` state in the record loop, so the off-by-default, local-only, peer-credential-gated security model is unchanged and it still carries no agent-facing text. See [docs/reference/control-channel.md](docs/reference/control-channel.md#skip-forward-behaviour).
...
- **Control channel — CD-style skip-back to replay the previous utterance (VM-1685)** — The control channel grows **CD transport** alongside the cassette-style pause/resume/stop: press **skip-back** to re-hear what the assistant just said. The first press restarts the current utterance from its start — **even mid-playback**: a press silences the device instantly (responsive barge-in) while the server keeps draining the rest of that utterance into a small **history buffer** in the background, so it restarts the *current* one from the top rather than jumping to the previous one. Each further press steps to the utterance before, walking back through the buffer — so on a walk you can ask "what was that bit again?" with a single button instead of asking the agent to repeat itself. It **composes with pause** — a pause mid-utterance likewise drains the remainder into the buffer (freeing the provider connection), so **resume plays the buffered remainder** instead of re-pulling the provider — and replays *cached audio only*: **no new agent turn, no STT, no model call** — so, like pause/resume/stop, it stays on the safe side and needs no special permission. The uninterrupted path still plays as audio arrives, so time-to-first-audio is unchanged. Drive it with the new `voicemode control skip-back` verb, a Stream Deck button, a media key, or a raw `{"command":"skip_back"}` line. The history buffer captures every utterance **regardless of `SAVE_AUDIO`**, is bounded in memory (`VOICEMODE_HISTORY_BUFFER_SIZE`, default 8), and a new request/response `{"command":"status"}` query reports "now playing" + buffer position for status displays (programmatic / raw-socket; `query_status()` in Python). Existing pause/resume/stop behaviour and the control-socket security model are unchanged. Skip-*forward* (deterministic barge-in to the listen turn) is its sibling task VM-1739; once it also lands, the Stream Deck `deck.py` `SKIP_ENABLED` flag flips to light both buttons. See [docs/reference/control-channel.md](docs/reference/control-channel.md).
- **`voicemode reconnect` — heal a dropped voice session in one call (VM-1727)** — When the voicemode MCP server drops mid-session (`-32000 Connection closed`) or fails to register its tools at launch, *its* MCP tools vanish — so the fix can't be one of them. The new `voicemode reconnect` CLI runs the whole `/mcp` reconnect dance from a single Bash call the agent can make itself: it opens the `/mcp` menu on its own tmux pane, finds the voicemode server **by name** (not a fragile keystroke count), navigates to it by reading the menu cursor (robust to the stale `❯ /mcp` prompt echoes that share the cursor glyph), hits **Reconnect** only if it's failed, re-opens the menu and polls until it reads connected, then prints the exact `ToolSearch` line to reload the converse schema. It's deliberately **MCP-independent** (standard library + Click only) so it imports and runs at the exact moment the server is down, and **fails loud** rather than sending blind keystrokes if the menu isn't what it expects. Distinct exit codes per outcome (`0` reconnected · `10` already-connected · `11` not-found · `12` timeout · `13` not-in-tmux · `1` error). Verified end-to-end against a real Claude Code v2.1.186 `/mcp` menu — it replaces the hand-driven `tmux send-keys` recipe with one command.

### Changed

- **Media-key "next" now barges in (`skip-forward`) instead of just stopping (VM-1742)** — The Hammerspoon media-key binding (VM-1724) mapped the Next / fast-forward key to `voicemode control stop`, which cut the assistant off but left the turn with the agent (no listening). Now that `skip-forward` exists (VM-1739), the key sends `voicemode control skip-forward`: pressing it mid-utterance cuts the assistant **and hands you the mic** — the true `>>|` barge-in, returning your spoken reply as an ordinary `Voice response: …`. Point `~/.hammerspoon/init.lua` at the canonical `scripts/hammerspoon/voicemode-media-keys.lua` (no longer a task worktree path) to pick it up.
- **Pause-and-walk-away: a held `pause` now waits 5 minutes, not 30 seconds, before self-healing (VM-1853)** — Pausing an in-flight `converse` utterance via the control channel (media key / Stream Deck) holds the global audio lock until you resume; a never-resumed pause self-heals after `VOICEMODE_CONTROL_PAUSE_TIMEOUT` (the F4 hardening cap). The default was **30 s**, which cut off a *deliberate* hold — a host pausing the assistant to step away and serve guests would come back to find playback already auto-stopped and the tool returned early. The default is now **300 s (5 min)**, comfortably covering that "pause, walk away, resume" flow while still preventing a forgotten/malicious pause from wedging the audio subsystem indefinitely. Still fully tunable per server via `VOICEMODE_CONTROL_PAUSE_TIMEOUT` (seconds; `0` disables the cap), and the timed-out action (`stop`/`resume`) via `VOICEMODE_CONTROL_PAUSE_TIMEOUT_ACTION` — unchanged.

### Security

- **Hardened the control channel before it's recommended for use (VM-1688)** — Following an adversarial review, the (off-by-default) control channel gained real access control and input safety so enabling it can't turn a stray local process into a prompt-injection vector against an agent that holds shell/file tools. **Named intents replace free-form text**: a `stop`'s agent-facing sentence is chosen from a server-owned allowlist (`switch-to-text`/`brevity`/`quiet`); any caller `message` is logged on the server only and never surfaced to the model (F1). **Peer-credential authentication** (`SO_PEERCRED`/`LOCAL_PEERCRED`) rejects other users; the socket directory is created `0700` and bound under `umask(0o077)` — the real gate, since socket-file mode isn't enforced on macOS/BSD (F2/F3). **Input is bounded** (8 KiB line, 256-char message) against memory/context flooding (F5); a **never-resumed `pause` self-heals** after `VOICEMODE_CONTROL_PAUSE_TIMEOUT` (default 300 s / 5 min — VM-1853) rather than wedging the audio lock (F4); stale-socket cleanup **refuses to unlink a non-socket it doesn't own** (F6); and handler threads / connection lifetime are capped (F7). The docs Security section was rewritten to the real threat model (F8).

### Fixed

- **Local Whisper STT retries a transient failure instead of losing the turn (VM-926)** — When the local whisper.cpp STT endpoint failed transiently — most often a read **timeout** — `converse` lost the whole voice turn and you had to repeat yourself. The STT client was built with `max_retries=0` for local providers (the "local either works or doesn't" assumption), and with a single local endpoint there was nothing to fail over to, so one timeout was a hard failure. Local STT transcription now **retries a transient failure a small number of times with a short exponential backoff** before giving up, so an intermittent timeout recovers on its own. Only **transient** errors retry (timeouts, connection errors, 5xx); **permanent** ones (4xx bad-audio/auth, 429 rate-limit) fail fast without looping. Tunable in `voicemode.env`: `VOICEMODE_STT_RETRY_ATTEMPTS` (default 2), `VOICEMODE_STT_RETRY_BACKOFF` (default 0.5s), `VOICEMODE_STT_RETRY_BACKOFF_MAX` (default 4.0s); set attempts to `0` to disable. Remote (OpenAI) STT behaviour is unchanged.
